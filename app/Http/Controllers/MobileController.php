...
class MobileController extends Controller
{
	...
	public function userinfos(Request $request){
		...
		$listeUtilisateurs[] = 
        	array(
				...
				'utilisateurPhoto' =>"http://crakgame.com/public/images/avatars/".$user->photo,
	...
	public function dataforofflineuse(){
		...
		foreach($questions as $question){
			...
			$listeQuestions[] = 
                	array(
                		'ID' =>$question->id,
						'QUESTION' =>stripslashes(utf8_encode($question->question_fr)),
		...
		echo json_encode(array('listeOperationResult'=>$listeOperationResult, 'listeCategories'=>$listeCategories, 'listeQuestions'=>$listeQuestions));
	...
	public function categories(Request $request){
		...
		echo json_encode(array('listeOperationResult'=>$listeOperationResult,'listeCategories'=>$listeCategories));
	...
	public function questions_tc(Request $request){
		...
		echo json_encode(array('listeOperationResult'=>$listeOperationResult,'listeQuestions'=>$listeQuestions));
	...
	public function duelquestions(Request $request){
		...
		echo json_encode(array('listeQuestions'=>$listeQuestions));
	...
	public function classements(Request $request){
		...
		if($type == 1){
			...
			foreach($users_duels_gagnes as $user){
				...
				$listeClassements[] = 
					array(
						...
						'classementPhoto' =>"http://crakgame.com/public/images/avatars/".$user->photo,
			...
			foreach($entrainements as $entrainement){
				...
				$listeClassements[] = 
					array(
						...
						'classementPhoto' =>"http://crakgame.com/public/images/avatars/".$entrainement->photo,
						'classementNote' =>'',
						'classementDate' =>"",
					);
			...
			$sql = 'select sum(entrainement_score) as entrainement_score, DATE(entrainement_date) as entrainement_date, pseudo, photo from entrainement inner join users on id = user_id 
			where 1 ';
	...
	public function transactions(Request $request){
			...
			foreach($transactions as $transaction){
				...
				$listeTransactions[] = 
					array(
						...
						'transactionPhoto' =>"http://crakgame.com/public/images/avatars/".$transaction->photo,
		...
		}else{//$type == 2 number_format($user->money, 0, ',', ' '),
	...
	public function updateuserinfos(Request $request){
		...
		if(!empty($user) ){
			...
			// $user->utilisateurPhoto = "http://crakgame.com/images/".$request->photo;
		...
		foreach($avatars as $avatar){
			...
			if($avatar!="." && $avatar !=".."){
				...
				$listeAvatars[] = 
					array(
						'avatarName' =>$avatar,
						'avatarPhoto' =>"http://crakgame.com/public/images/avatars/".$avatar,
					);
